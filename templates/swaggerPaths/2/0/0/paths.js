...
module.exports = {
  ...
  schemes: '"schemes": {{#schemes}}{{&process}}, {{/schemes}}{{^schemes}}[], {{/schemes}}', // eslint-disable-line max-len
  ...
  operations: '{{#operations}}"{{operationName}}": {{=<% %>=}}{<%={{ }}=%> {{> tags}} {{#summary}}"summary": "{{summary}}",{{/summary}} {{#description}}"description": "{{description}}",{{/description}} {{> externalDocs}} {{#operationId}}"operationId": "{{operationId}}", {{/operationId}} {{#deprecated}}"deprecated": {{deprecated}}, {{/deprecated}} {{> consumes}} {{> produces}} {{> parameters}} {{> schemes}} {{> security}} "responses": {{> responses}} {{=<% %>=}}}<%={{ }}=%>{{#comma}},{{/comma}}{{/operations}}', // eslint-disable-line max-len
