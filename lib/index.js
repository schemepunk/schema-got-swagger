...
module.exports = class SchemaGotSwagger {
  ...
  init(
    ...
    const getterDefaults                         = new GetDefaults('sgs'); // eslint-disable-line max-len
    const realizationOrder                                  = [];
    this.definitions = {};
    ...
    return getterDefaults.getDefaults()
      ...
      .then((sgsMergedConfig) => {
        ...
        const dataSps                                                                                             = { // eslint-disable-line max-len
      ...
      .then((dataSpsInitialized) => {
        ...
        const swaggerSrcTemplates                                    = this.spMaker(templatesData, 'templates', 'swaggerSrc', swaggerSrcOptions); // eslint-disable-line max-len
        const templatesPaths = _.get(pathItemsSrcOptions, ['templates', 'templates'], {});
        const pathsTemplates                                    = this.spMaker(templatesPaths, 'templates', 'paths', pathItemsSrcOptions); // eslint-disable-line max-len
        const schemesData = _.get(swaggerSrcOptions, ['schemes', 'schemes'], {});
        const swaggerSrcSchemes                                        = this.spMaker(schemesData, 'schemes', 'swaggerSrc', swaggerSrcOptions); // eslint-disable-line max-len
        const schemesPaths = _.get(pathItemsSrcOptions, ['schemes', 'schemes'], {});
        const pathsSchemes                                        = this.spMaker(schemesPaths, 'schemes', 'paths', pathItemsSrcOptions); // eslint-disable-line max-len
        return Promise.all([
   ...
   */
  setConfig(config           )                   {
    const validConfig            = sgsValidator(config);
    if (!validConfig) {
   ...
   *
   */
  integrateSwaggerSrcTemplatesAndSchemes()                   {
   ...
   *
   */
  integratePathsTemplatesAndSchemes()                   {
   ...
   */
  pathsEntitySpGuarantee(currentSemverArray               , dataAttributesAtLevel    , spClassToGuarantee                        ) { // eslint-disable-line max-len
    const pathsAttributesArray                = Object.keys(dataAttributesAtLevel);
    ...
    const spAttributeNamesArray                = _.keys(_.get(
   ...
   *  An instance of this.
   */
  setMainDataSpClass(mainDataSp                                      )                   { // eslint-disable-line max-len
   ...
   */
  setPathsDataSpClass(pathsDataSp                                )                   { // eslint-disable-line max-len
   ...
   *   Returns an instance of this.
   */
  setDesiredRealizations(realizations               )                   {
   ...
   *   Returns an instance of this class.
   */
  setSwaggerSrcTemplatesSpClass(templatesSp                                   )                   {
   ...
   *   Returns an instance of this class.
   */
  setPathsTemplatesSpClass(templatesSp                                   )                   {
   ...
   *   Returns an instance of this class.
   */
  setSwaggerSrcSchemesSpClass(schemesSp                                       )                   {
   ...
   *   Returns an instance of this class.
   */
  setPathsSchemesSpClass(schemesSp                                       )                   {
   ...
   */
  semverStringSplit(semverString        ) {
