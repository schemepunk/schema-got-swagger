...
module.exports = class SchemaGotSwagger {
  ...
  mainSwaggerTemplates: semverish
  mainSwaggerTemplatesComposer: *
  mainSwaggerSchemes: (semverish | SchemaGotSwagger | {})
  mainSwaggerSchemesComposer: *
  ...
  init(swaggerSrc: semverishSrc, pathItemsSrc: semverish, config: userSgsConfig = {}, swaggerSrcOptions?: mainSwaggerMakerOptions, pathItemsSrcOptions: pathSwaggerMakerOptions) { // eslint-disable-line max-len
    ...
    return this.setConfig(config)
      ...
      // setTemplates - get value passed and run through semverize parameters
      // setTemplatesComposer - run semverist on templates to get
      // templates composer.
      .then(() => this.setMainSwaggerTemplates(_.get(
        swaggerSrcOptions,
        ['templates', 'templateOverrides'],
        {}
      )))
      .then(() => this.setMainSwaggerTemplatesComposer())
      // Set schemes - set value passed and run through semverize parameters
      // setSchemeComposer - run schemes on
      .then(() => this.setMainSwaggerSchemes(_.get(swaggerSrcOptions, ['schemes', 'schemes'], {})))
      .then(() => this.setMainSwaggerSchemesComposer())
  ...
  /**
   * Sets main swagger templates.
   *
   * @param {(semverish | mainTemplate | {})} data
   *   A semverish, maintemplate or empty object passed
   *   to represent the templates you wish to use for the
   *   main swagger data shape.
   * @returns {SchemaGotSwagger}
   *   returns a schema got swagger instance.
   */
  setMainSwaggerTemplates(data) {
    return this.semverizeParameters(
      'SwaggerMainTemplates',
      data,
    )
      .then((semverized) => {
        this.mainSwaggerTemplates = semverized;
        return this;
      });
  }

  /**
   * Get the semverish templates.
   *
   * @returns {semverish}
   *   Returns a semverish bearing templates for the main swagger.
   */
  getMainSwaggerTemplates(): semverish {
    return this.mainSwaggerTemplates;
  }

  /**
   * Sets a main swagger templates composer.
   *
   * @returns {SchemaGotSwagger}
   *   Returns a instance of Sgs
   */
  setMainSwaggerTemplatesComposer() {
    return SchemaGotSwagger.createComposer(
      this.getMainSwaggerTemplates(),
      { swagger: this.getSwaggerMainSemveristTemplatesConfig() },
      'swagger',
      this.getSwaggerMainTemplatesMolotov(),
    )
      .then((value) => {
        this.mainSwaggerTemplatesComposer = value;
        return this;
      });
  }

  /**
   * Set the main swagger Schmes from parameters.
   *
   * @param {(semverish | schemePunkConfig | {})} data
   *   Passed in schemes.
   * @returns {SchemaGotSwagger}
   *   Returns an instance of this.
   */
  setMainSwaggerSchemes(data: (semverish | schemePunkScheme | {})): Promise<SchemaGotSwagger> {
    return this.semverizeParameters(
      'SwaggerSrcSchemes',
      data,
    )
      .then((semverized) => {
        this.mainSwaggerSchemes = semverized;
        return this;
      });
  }

  /**
   * Gets the main swagger scheme semverish.
   *
   * @returns {semverish}
   *   Returns a scheme bearing semverish.
   */
  getMainSwaggerSchemes() {
    return this.mainSwaggerSchemes;
  }

  /**
   * Set the main swagger schemes composer.
   *
   * @returns {*}
   *   Returns a composer class for the main swagger schemes.
   */
  setMainSwaggerSchemesComposer() {
    return SchemaGotSwagger.createComposer(
      this.getMainSwaggerSchemes(),
      { swagger: this.getSwaggerMainSemveristSchemesConfig() },
      'swagger',
      this.getSwaggerMainSchemesMolotov('semverist'),
    )
      .then((value) => {
        this.mainSwaggerSchemesComposer = value;
        return this;
      });
  }
   ...
   */
  semverizeParameters(
    sgsDefaultType: ('SwaggerMainTemplates' | 'SwaggerSrcSchemes'),
    value: {}
  ) {
    ...
    if (makeSemverish) {
      ...
      tmpValue = this.mergeConfig(value, sgsDefaultType, this.getConfig())
        .then((preparedParameter) => {
          const testParam = swaggerMainSemveristSrcValidator(preparedParameter);
          if (!testParam) {
            Promise.reject(new SchemaGotSwaggerError(testParam.err));
          }
          return this.semverizeToRealizations(preparedParameter, 'swagger');
        });
    }
    else {
      // This is either a semverist shape or semverish.
      // We will pass
      tmpValue = Promise.resolve(value);
    }
    return tmpValue;
  }
  ...
  setSwaggerSrcConfig(swaggerSourceOptions: (mainSwaggerMakerOptions | {}) = {}) {
    ...
    const mainSwaggerTemplatesConfig = _.get(
      swaggerSourceOptions,
      ['templates', 'semveristConfig'],
      {}
    );
    const mainSwaggerSchemesConfig = _.get(
      swaggerSourceOptions,
      ['schemes', 'semveristConfig'],
      {}
    );
    const mainSemveristSrcMolotov = _.get(
      swaggerSourceOptions,
      ['data', 'semveristMolotovOptions'],
      {}
    );
    const mainSwaggerTemplatesMolotov = _.get(
      swaggerSourceOptions,
      ['templates', 'semveristMolotovOptions'],
      {}
    );
    const mainSwaggerSchemesSemveristMolotov = _.get(
      swaggerSourceOptions,
      ['schemes', 'semveristMolotovOptions'],
      {}
    );
    const mainSwaggerSchemesSchemePunkMolotov = _.get(
      swaggerSourceOptions,
      ['schemes', 'schemePunkMolotovOptions'],
      {}
    );
   ...
   */
  setSwaggerMainSemveristTemplatesConfig(swaggerMainSemveristTemplatesConfig: Promise<SchemaGotSwagger>) { // eslint-disable-line max-len
   ...
   *   An instance of this.
   */
  setSwaggerMainSemveristSchemesConfig(swaggerMainSemveristSchemesConfig: semveristConfig): Promise<SchemaGotSwagger> { // eslint-disable-line max-len
    return this.mergeConfig(
  ...
  mergeConfig(
    ...
    const tmpConfig = Object.keys(config).length ? Promise.resolve(config) : Promise.resolve({}); // eslint-disable-line max-len
  ...
  setSwaggerSrc(swaggerSrc: semverish) {
    ...
    const validSemverish = swaggerMainSemveristSrcValidator(swaggerSrc);
